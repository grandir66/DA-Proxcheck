...
- **Essenziale / Base / Completo**: l'essenziale sono i dati senza cui non si progetta e i parametri che possono bloccare la migrazione (57 domande e quattro tabelle ridotte); la base è per installazioni da 2-3 nodi; la completa aggiunge inventario host, SAS diretto, dipendenze fra servizi, conformità estesa. I tre livelli sono annidati: si comincia dal minimo e si allarga senza ribattere niente.
...
- **Esito**: un pannello in cima al documento — e una sezione in testa al Markdown — che giudica *le risposte date*, non la loro presenza. Trenta regole dicono cosa comporta una certa risposta e cosa serve per toglierla di mezzo: VM su vSAN, storage senza UNMAP, Veeam senza storage file-level, due nodi senza QDevice, BitLocker senza recovery key. Un blocco non impedisce di esportare: va consegnato al cliente, non nascosto.
- **Le regole si vedono, non si citano soltanto**: ogni rilievo porta il rimando al manuale operativo Domarc, e in coda al documento «Le regole applicate» riporta il testo per esteso di ogni passaggio richiamato, con la sua origine. Come il report dell'audit, il questionario compilato basta a sé stesso. Il testo è estratto dal manuale da `strumenti/estrai-fonti.py` e vive dentro la pagina: funziona offline.
...
- **`questionario/questionario-migrazione.html`** — a single-file, offline **migration questionnaire** (VMware → Proxmox or greenfield), Italian and English, **Essential/Basic/Full** scope, per-node IP table, defaults, auto-saved draft and **Markdown export**. Thirty rules judge the *answers* — not just their presence — and the **Outcome** panel says which of them stop the migration and what it takes to clear each one. Every rule cites Domarc's Proxmox VE manual and the exported document **quotes the cited passages in full**, so it stands on its own. Published at <https://grandir66.github.io/DA-Proxcheck/questionario/questionario-migrazione.html>.
