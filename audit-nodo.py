...
PROFILI = {
    "1": {"nome": "Domain controller / DNS", "vcpu_max": 4, "balloon": "min_eq_max",
          ...
          "protection": True, "ha": True, "fonte": "manuale §9.1, §11.17",
          "extra": ["Almeno un secondo domain controller su un altro nodo fisico.",
                    ...
                    "Driver VirtIO: 0.1.271 sui Windows moderni; dopo un cambio, replica AD e login verificati (§11.17)."]},
    "2": {"nome": "Database", "vcpu_max": None, "balloon": "disabilitato",
          "cpu_type_evita": {"kvm64", "x86-64-v2-AES"}, "numa": True, "cache": "none", "multiqueue": "no",
          "protection": True, "ha": True, "dischi_min": 2, "fonte": "manuale §9.4, §11.17",
          "extra": ["Dischi dati e log/WAL su dischi virtuali separati.",
                    "aio=native SOLO con raw + cache=none + iothread; altrimenti io_uring.",
                    "Driver VirtIO su Windows: MAI 0.1.285/0.1.292 su Server 2025 con SQL Server (errori di lettura "
                    "sotto carico); 0.1.271, e una versione nuova solo dopo prove I/O col carico vero (§11.17)."]},
    "3": {"nome": "Applicativo / web / monitoraggio", "vcpu_max": 8, "balloon": "attivo_salvo_java",
          "cpu_type_evita": {"kvm64"}, "numa": False, "cache": "none", "multiqueue": "no",
          "protection": False, "fonte": "manuale §9.3, §9.8, §11.17",
          "extra": ["Con JVM (Zabbix Java gateway, Elastic, Tomcat) il ballooning va valutato: la JVM non restituisce memoria.",
                    "Driver VirtIO su Windows: 0.1.271; una versione nuova si collauda su rete e latenza contro la baseline (§11.17)."]},
    ...
    "5": {"nome": "Dati: file server, log/SIEM, backup server", "vcpu_max": None, "balloon": "attivo_min_alto",
          ...
          "protection": True, "dischi_min": 2, "fonte": "manuale §9.2, §9.6, §12.1, §11.17",
          "extra": ["Disco dati separato dal disco di sistema (backup e throttling distinti).",
                    "Driver VirtIO su Windows: 0.1.271; dopo un cambio, TRIM verificato (Optimize-Volume -ReTrim) e spazio che torna allo storage (§11.17).",
    ...
    "6": {"nome": "Terminal server / VDI", "vcpu_max": None, "balloon": "attivo", "cpu_type_evita": set(),
          ...
          "protection": False, "fonte": "manuale §9.7, §11.17",
          "extra": ["Driver VirtIO su Windows: 0.1.271; su LTSB/LTSC vecchie il driver grafico (viogpudo) si gestisce a parte, resta a 0.1.240 (§11.17)."]},
    "7": {"nome": "Test, legacy, replica", "vcpu_max": None, "cpu_type_evita": set(), "numa": False,
          ...
          "fonte": "manuale §8.6, §9.9, §4.5, §11.17",
          "extra": ["Test: non condividere rete/VLAN con la produzione se il traffico non è filtrato.",
                    ...
                    "Windows legacy: l'ultima virtio-win che CONTIENE il sistema — 2003 0.1.100, 2008 0.1.137, "
                    "2008 R2 0.1.173-4, 2012/R2 0.1.189 (mai 0.1.215-0.1.262); non si aggiorna, si isola (§11.17).",
